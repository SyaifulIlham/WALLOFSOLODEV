...
const ManageSeats = () => {
    ...
    const [editSeat, setEditSeat]     = useState(null); // null = tambah, object = edit
    ...
    const seatsByRow = useMemo(() => {
        ...
        // Sort tiap baris berdasarkan nomor
        ...
        <AdminLayout activeMenu="seats">
            <div className="container-fluid">
                ...
                <style>{`
                    ...
                    .form-input { width:100%; background:#0f1726; border:1px solid rgba(255,255,255,0.1); border-radius:10px; padding:10px 14px; color:#fff; font-size:0.9rem; outline:none; }
                ...
                <div className="p-5 rounded-4" style={{ background: '#111827', border: '1px solid rgba(148,163,184,0.12)' }}>
                    ...
                    <div className="d-flex gap-3 mb-4" style={{ flexWrap: 'wrap' }}>
                        ...
                        <div className="d-flex gap-2 ms-auto align-items-center">
                            <button className="view-btn" onClick={() => setViewMode('grid')} style={{ backgroundColor: viewMode === 'grid' ? 'rgba(59,130,246,0.2)' : 'rgba(255,255,255,0.04)', color: viewMode === 'grid' ? '#60a5fa' : '#94a3b8' }}>
                                🎬 Grid
                            </button>
                            <button className="view-btn" onClick={() => setViewMode('table')} style={{ backgroundColor: viewMode === 'table' ? 'rgba(59,130,246,0.2)' : 'rgba(255,255,255,0.04)', color: viewMode === 'table' ? '#60a5fa' : '#94a3b8' }}>
                                📋 Tabel
                            </button>
                        </div>
                    </div>
                    ...
                    {loading ? (
                        ...
                        <div>
                            ...
                            <div style={{ textAlign: 'center', marginBottom: 32 }}>
                                <div style={{ display: 'inline-block', padding: '8px 80px', background: 'linear-gradient(180deg,rgba(96,165,250,0.25),transparent)', borderRadius: '0 0 60% 60% / 0 0 30px 30px', color: '#93c5fd', fontSize: '0.78rem', fontWeight: 700, letterSpacing: '3px', marginBottom: 4 }}>
                                    LAYAR
                                </div>
                            ...
                            <div className="d-flex gap-4 justify-content-center mb-4">
                                {[
                                    { label: 'Tersedia', color: '#4ade80', bg: 'rgba(74,222,128,0.15)' },
                                    { label: 'Dipesan',  color: '#f87171', bg: 'rgba(248,113,113,0.15)' },
                                ].map(l => (
                                        ...
                                        <div className="d-flex gap-2">
                                            {seatsByRow[baris].map(seat => (
                                                <div
                                                    ...
                                                    title={`${seat.nomor_kursi} — ${seat.status} (klik untuk ubah status)`}
                                                >
                                                    {seat.nomor_kursi}
                                                </div>
                                            ))}
                            ...
                            <p style={{ textAlign: 'center', color: '#475569', fontSize: '0.78rem', marginTop: 16 }}>
                                💡 Klik kursi untuk mengubah status tersedia ↔ dipesan
                            </p>
                        </div>
                                    ...
                                    <div key={seat.id_seat} className="row px-4 py-3 align-items-center admin-row">
                                        ...
                                        <div className="col-3">
                                            <button
                                                onClick={() => toggleStatus(seat)}
                                                style={{
                                                    padding: '5px 14px', borderRadius: 20, border: 'none', fontWeight: 700, fontSize: '0.8rem', cursor: 'pointer',
                                                    backgroundColor: seat.status === 'tersedia' ? 'rgba(74,222,128,0.15)' : 'rgba(248,113,113,0.15)',
                                                    color: seat.status === 'tersedia' ? '#4ade80' : '#f87171',
                                                }}
                                            >
                    ...
                    <div className="modal-overlay" onClick={() => setShowModal(false)}>
                        <div className="modal-box" onClick={e => e.stopPropagation()}>
                            ...
                            <div className="d-flex flex-column gap-3">
                                <div>
                                    <label style={{ color: '#94a3b8', fontSize: '0.82rem', marginBottom: 6, display: 'block' }}>Nomor Kursi</label>
                                    <input className="form-input" placeholder="Contoh: A1" value={form.nomor_kursi}
                                        onChange={e => setForm({ ...form, nomor_kursi: e.target.value })} />
                                </div>
                                <div>
                                    <label style={{ color: '#94a3b8', fontSize: '0.82rem', marginBottom: 6, display: 'block' }}>Baris</label>
                                    <input className="form-input" placeholder="Contoh: A" value={form.baris}
                                        onChange={e => setForm({ ...form, baris: e.target.value.toUpperCase() })} maxLength={2} />
                                </div>
                                <div>
                    ...
                    <div className="modal-overlay" onClick={() => setShowGenerate(false)}>
                        <div className="modal-box" onClick={e => e.stopPropagation()}>
                            ...
                            <p style={{ color: '#64748b', fontSize: '0.85rem', marginBottom: 24 }}>
                                Contoh: A s/d E, 10 kursi per baris → A1–A10, B1–B10, ... E1–E10
                            </p>
                            <div className="d-flex flex-column gap-3">
                                <div className="d-flex gap-3">
                                    <div style={{ flex: 1 }}>
                                        <label style={{ color: '#94a3b8', fontSize: '0.82rem', marginBottom: 6, display: 'block' }}>Baris Awal</label>
                                        <input className="form-input" value={genForm.baris_awal} maxLength={1}
                                            onChange={e => setGenForm({ ...genForm, baris_awal: e.target.value.toUpperCase() })} />
                                    </div>
                                    <div style={{ flex: 1 }}>
                                        <label style={{ color: '#94a3b8', fontSize: '0.82rem', marginBottom: 6, display: 'block' }}>Baris Akhir</label>
                                        <input className="form-input" value={genForm.baris_akhir} maxLength={1}
                                            onChange={e => setGenForm({ ...genForm, baris_akhir: e.target.value.toUpperCase() })} />
                                    </div>
                                </div>
                                <div>
                                    <label style={{ color: '#94a3b8', fontSize: '0.82rem', marginBottom: 6, display: 'block' }}>Jumlah Kursi per Baris</label>
                                    <input className="form-input" type="number" min={1} max={30} value={genForm.jumlah_per_baris}
                                        onChange={e => setGenForm({ ...genForm, jumlah_per_baris: e.target.value })} />
                                </div>
                                <div className="d-flex gap-2 mt-2">
