...
// 2. POST FILM (Tambah Film) - SUDAH ADA STATUS
router.post('/films', async (req, res) => {
  try {
    // Ambil 'status' dari body
    const { judul_film, id_kategori, genre, durasi, deskripsi, poster, status } = req.body;

    // Masukkan 'status' ke query. created_by kita set manual ke 1 (Syaiful) agar aman.
...
// 5. PUT FILM (Update Film) - SUDAH ADA STATUS
router.put('/films/:id', async (req, res) => {
    try {
        const { judul_film, id_kategori, genre, durasi, deskripsi, poster, status } = req.body;
        const idFilm = req.params.id;

        // Tambahkan 'status = ?' di query UPDATE
        const query = `
            UPDATE films 
...
// 6. GET ADMIN (Ambil data admin untuk nama dinamis di Dashboard)
router.get('/admins', async (req, res) => {
    try {
        // Mengambil 1 data admin dari tabel admins
